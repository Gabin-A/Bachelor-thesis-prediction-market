...
# Data transforms
# -----------------------------
def timeseries_from_bets(bets):
    rows = []
    for b in bets:
        if "probAfter" in b and b.get("createdTime") is not None:
            rows.append((b["createdTime"], b["probAfter"], float(b.get("amount") or 0.0)))
    if not rows:
        return None
    df = pd.DataFrame(rows, columns=["t_ms", "p", "amount"]).sort_values("t_ms").drop_duplicates("t_ms")
    df["t"] = pd.to_datetime(df["t_ms"], unit="ms", utc=True)
    return df[["t", "p", "amount"]].reset_index(drop=True)

def resample_probs_and_flow(ts, freq="1h"):
    df = ts.set_index("t").sort_index()
    p = df["p"].resample(freq).last().ffill()
    n_bets = df["p"].resample(freq).size().fillna(0)
    amt = df["amount"].resample(freq).sum().fillna(0)

    out = pd.DataFrame({"p": p, "n_bets": n_bets, "amt": amt}).reset_index()
...
def build_panel(prob_df, k_steps, h_steps):
    ...
    n_bets = prob_df["n_bets"].to_numpy()
    ...
    # attention proxies summed over the spike window
    attn_bets = np.array([n_bets[i - k_steps + 1 : i + 1].sum() for i in idx])
...
def main():
    ...
    for _, m in tqdm(markets.iterrows(), total=len(markets)):
        ...
        ts = timeseries_from_bets(bets)
        if ts is None or len(ts) < 30:
            continue

        rs = resample_probs_and_flow(ts, freq=RESAMPLE_FREQ)
    ...
    # Attention mechanism
    data["log_attn_bets"] = np.log1p(data["attn_bets"])
    data["dp0_x_attn"] = data["dp0"] * data["log_attn_bets"]

    # FE: demean time-varying columns
    fe = demean_by_group(data, "market_id", ["dph", "dp0", "dp0_x_attn"])
    ...
    print("\n=== FE + Cluster(market): dph ~ dp0 + dp0×log(1+attn_bets) ===")
    print(m.summary())

    print("\nInterpretation guide:")
    print("- dp0 < 0: reversal/overreaction.")
    print("- dp0_x_attn < 0: higher attention strengthens reversal (attention-driven overreaction).")
